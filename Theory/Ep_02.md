...
A: `npx` is a tool that is used to execute the packages. It comes with the npm, when you installed npm above 5.2.0 version then automatically npx get installed. It is an npm package runner that can execute any package that you want from the npm registry without even installing that package. For eg- we use `npm`
keyword in a command to download or install some library or package `npm i parcel` but npx keyword is used to execute that lib or package `npx parcel index.html`. `x` in `npx` stands for execute.
...
- Application name/version/scripts/dependencies
